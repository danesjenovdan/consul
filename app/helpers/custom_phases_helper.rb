module CustomPhasesHelper
  ...
  def custom_phases_links(custom_phases)
    ...
    if current_phase === 'accepting' || current_phase === 'reviewing'
      ...
      custom_phases[CUSTOM_PHASE_BALLOTING].url = budget_investments_url(current_budget, heading_id: current_user  && current_user.balloted_heading_id ? current_user.balloted_heading_id : current_budget.headings.first.id)
