...
K8S_NAMESPACE="consul-staging"
SECRETS_NAME="consul-credentials"

# DATABASE PASSWORD IS DYNAMICALLY RETRIEVED FROM THE CLUSTER
DATABASE_USER=$(kubectl get secret $SECRETS_NAME -n $K8S_NAMESPACE -o jsonpath="{.data.POSTGRES_USERNAME}" | base64 --decode)
DATABASE_PASSWORD=$(kubectl get secret $SECRETS_NAME -n $K8S_NAMESPACE -o jsonpath="{.data.POSTGRES_PASSWORD}" | base64 --decode)
...
# docker container exec -i $(docker compose ps -q database) psql -U postgres consul < db.dump
